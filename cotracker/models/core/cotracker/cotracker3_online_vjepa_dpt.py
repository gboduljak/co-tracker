...
from cotracker.models.core.cotracker.vjepa import VJEPADPTExtractor
from einops import rearrange
...
class CoTrackerThreeBase(nn.Module):
    def __init__(
        ...
        self.vjepa = VJEPADPTExtractor(
            vjepa=vjepa,
            height=h,
            width=w,
            vjepa_intermediate_layers=vjepa_intermediate_layers,
            vjepa_dim=vjepa_channels,
            latent_dim=self.latent_dim,
            down_ratio=stride,
            freeze_vjepa=freeze_vjepa
        )
        self.input_dim = 1110
    ...
    def get_video_feat(self, video):
        f = self.vjepa(video)
        f = rearrange(f, "b t d h w -> (b t) d h w")
        return f
